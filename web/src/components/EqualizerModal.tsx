...
import { motion, HoverScale, ModalOverlay } from '../components/motion';
      ...
      >
        ...
        <div style={{ marginBottom: '16px', opacity: eqEnabled ? 1 : 0.35, pointerEvents: eqEnabled ? 'auto' : 'none', transition: 'opacity 0.3s' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            {Object.keys(PRESETS).map(p => (
              <HoverScale key={p} scale={1.05} tapScale={0.95}>
                <button
          ...
          <div style={{
            ...
            {eqBands.map((gain, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, height: '100%', gap: '6px' }}>
                <span style={{ fontSize: '9px', fontWeight: 700, color: gain > 0 ? '#A78BFA' : gain < 0 ? '#60A5FA' : '#4B5563', minHeight: '12px', lineHeight: 1 }}>
                ...
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
                  <input
                    ...
                    style={{ writingMode: 'vertical-lr' as const, direction: 'rtl' as const, WebkitAppearance: 'slider-vertical' as any, width: '6px', height: '75px', cursor: 'pointer', accentColor: '#7C3AED' }}
                  />
                </div>
                <span style={{ fontSize: '8px', color: '#374151', fontWeight: 700, textAlign: 'center' }}>{BAND_LABELS[i]}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ opacity: eqEnabled ? 1 : 0.35, pointerEvents: eqEnabled ? 'auto' : 'none', transition: 'opacity 0.3s' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <p style={{ fontSize: '9px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', color: '#4B5563' }}>Bass Boost</p>
            <span style={{ fontSize: '12px', fontWeight: 800, color: bassBoost > 50 ? '#10B981' : '#6B7280' }}>{bassBoost}%</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '10px', padding: '8px 12px' }}>
            <span style={{ fontSize: '14px' }}>🔈</span>
            <input type="range" min="0" max="100" value={bassBoost} onChange={e => setBassBoost(parseInt(e.target.value))} style={{ flex: 1, accentColor: '#10B981' }} />
            <span style={{ fontSize: '14px' }}>🔊</span>
          </div>
        </div>
      </div>
